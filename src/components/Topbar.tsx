  ...
  return (
    <header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-border bg-surface/90 px-4 backdrop-blur dark:border-white/10 dark:bg-ink/90">
      <div className="flex items-center gap-3">
        <button onClick={onMenuClick} className="text-ink-soft dark:text-white/60 lg:hidden" aria-label="Open menu">
          <Menu size={20} />
        </button>
        <h1 className="font-display text-base font-semibold text-ink dark:text-white">{title}</h1>
      </div>

      <div className="flex items-center gap-1">
        <button
          onClick={() => dispatch(toggleTheme())}
          className="rounded-md p-2 text-ink-soft hover:bg-paper dark:text-white/60 dark:hover:bg-white/5"
        ...
        >
          ...
          <span className="hidden text-left sm:block">
            ...
            <span className="block text-xs leading-tight text-ink-faint">{user?.role}</span>
          </span>
